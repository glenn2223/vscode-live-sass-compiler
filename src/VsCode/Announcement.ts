...
export async function checkNewAnnouncement(memento: Memento): Promise<void> {
    ...
    const showAnnoucement =
        Settings.getConfigSettings<boolean>("showAnnouncements");

    OutputWindow.Show(
        OutputLevel.Trace,
        "Checking `showAnnouncements` setting",
    );
    ...
    OutputWindow.Show(
        OutputLevel.Trace,
        "`showAnnouncements` is true so checking for an unread announcement",
    );
    ...
    if (
        ...
        if (choice === showMore) {
            const url =
                announment.url ||
                "https://github.com/glenn2223/vscode-live-sass-compiler/releases";
